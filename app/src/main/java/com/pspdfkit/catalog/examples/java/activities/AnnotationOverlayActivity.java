...
public class AnnotationOverlayActivity extends PdfActivity implements ToolbarCoordinatorLayout.OnContextualToolbarLifecycleListener {
    ...
    @NonNull private final List<Annotation> overlaidAnnotations = new ArrayList<>();

    /** Current strategy used for rendering annotations in overlay. */
    @NonNull
    private AnnotationOverlayRenderStrategy.Strategy currentOverlayRenderingStrategy = AnnotationOverlayRenderStrategy.Strategy.AP_STREAM_RENDERING;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // We'll set overlay render strategy that just returns currently configured strategy for all annotations.
        requirePdfFragment().setAnnotationOverlayRenderStrategy(annotation -> currentOverlayRenderingStrategy);

        // We'll enable overlay for all supported annotation types immediately after activity creation.
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        if (itemId == R.id.toggle_annotation_overlay) {
            ...
            return true;
        } else if (itemId == R.id.fire_low_memory_notification) {// We fire low memory notification manually to showcase how annotation overlay mode behaves when system is low on memory.
    ...
    protected void onResume() {
        super.onResume();
        // Register toolbar listener so we can add custom buttons for enabling/disabling overlay for selected annotations.
    ...
    public boolean onContextItemSelected(@NonNull MenuItem item) {
        ...
        final Annotation annotation = requirePdfFragment().getSelectedAnnotations().get(0);
