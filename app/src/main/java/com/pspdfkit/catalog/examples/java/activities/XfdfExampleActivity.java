...
public class XfdfExampleActivity extends PdfActivity {
    ...
    /**
     * Fires up {@link Intent#ACTION_CREATE_DOCUMENT} to allows user to pick output file for XFDF export.
     */
    ...
    /**
     * Fires up {@link Intent#ACTION_OPEN_DOCUMENT} to allows user to pick input file for XFDF import.
     */
     ...
     */
    private void exportToXfdf(@NonNull Uri uri) {
        try {
            final OutputStream outputStream = getContentResolver().openOutputStream(uri);
            if (outputStream == null) return;

            Completable writeXfdfCompletable = Completable.fromAction(() -> {
                PdfDocument document = getDocument();
                List<Annotation> allAnnotations = document.getAnnotationProvider().getAllAnnotationsOfTypeAsync(EnumSet.allOf(AnnotationType.class))
                    .toList()
                    .blockingGet();
                XfdfFormatter.writeXfdf(document, allAnnotations, Collections.<FormField>emptyList(), outputStream);
            });
            writeXfdfCompletable
                ...
                .subscribe(
                    () -> Toast.makeText(XfdfExampleActivity.this, "Annotations successfully exported", Toast.LENGTH_LONG).show(),
                    throwable -> Toast.makeText(XfdfExampleActivity.this, "Annotations export failed", Toast.LENGTH_LONG).show());
     ...
     */
    private void importFromXfdf(@NonNull Uri uri) {
        XfdfFormatter.parseXfdfAsync(getDocument(), new ContentResolverDataProvider(uri))
            ...
            .subscribe(annotations -> {
                ...
                Toast.makeText(XfdfExampleActivity.this, "Annotations successfully imported", Toast.LENGTH_LONG).show();
            }, throwable -> Toast.makeText(XfdfExampleActivity.this, "Annotations import failed", Toast.LENGTH_LONG).show());
    }
}
