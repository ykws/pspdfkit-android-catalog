...
/**
 * An example activity that demonstrates how to take the {@link PdfThumbnailGrid}, add a custom
 * view to it, and connect it with the {@link PdfProcessor} and {@link DocumentCoordinator} to create
 * an example where we can filter pages. This example incorporates the filter with choices to show
 * all pages, just annotated pages, or just bookmarked pages.
 *
...
public class FilterableThumbnailGridActivity extends PdfActivity implements FilterPickerView.OnFilterClickedListener {
    ...
    @Nullable private FilterPickerView filterPickerView = null;

    /**
     * In this example, we use this {@link androidx.lifecycle.ViewModel} to store the original (unfiltered)
     * document across configurations changes and reloads. View Model is part of the new Android
     * architecture components and is currently the way to go when it comes to restoring the state
     * across configuration changes (among other things).
     * @see <a href="https://developer.android.com/topic/libraries/architecture/viewmodel">ViewModel - Developers Guide</a>
     */
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        viewModel = new ViewModelProvider(this, new ViewModelProvider.NewInstanceFactory()).get(FilterableThumbnailGridViewModel.class);
     ...
     * @param thumbnailGridView The thumbnail grid view in which to add the {@link FilterPickerView}.
     * @param selectedFilter The default filter that will be selected.
     */
    private void generateFilterView(@NonNull PdfThumbnailGrid thumbnailGridView,
                                    @NonNull Filter selectedFilter) {
        // Generate only once.
        ...
        ViewCompat.setElevation(filterPickerView, getResources().getDimension(R.dimen.pspdf__thumbnailGridFilterPickerViewElevation));

        // Add view to the thumbnail grid (which is a RelativeLayout).
        thumbnailGridView.addView(filterPickerView, RelativeLayout.LayoutParams.MATCH_PARENT, RelativeLayout.LayoutParams.WRAP_CONTENT);
    ...
    public void onFilterClicked(@NonNull Filter filter) {
        if (filter == ALL) {
            ...
            if (originalDocument != null) {
                ...
                originalDocument.getAnnotationProvider()
                    // Get all annotations.
                    .getAllAnnotationsOfTypeAsync(EnumSet.allOf(AnnotationType.class))
                    // Map annotations to the page index they have (that's the only thing we need for filtering).
                    ...
                    .subscribe(new SingleObserver<List<Integer>>() {
                        ...
                        public void onSubscribe(Disposable d) {
                            // Do something once the filtering process starts. Not implemented in this example.
                        }

                        @Override
                        public void onSuccess(List<Integer> integers) {
                            if (integers.isEmpty()) { // If there are no annotations, we don't have any document to switch to.
                                Toast.makeText(FilterableThumbnailGridActivity.this, "There are no annotated pages in the document.", Toast.LENGTH_SHORT).show();
                                return;
                            }
                            ...
                            // Filter the original document to contain just the pages with annotations, and load it.
            ...
            if (originalDocument != null) {
                ...
                originalDocument.getBookmarkProvider()
                    // Get all bookmarks.
                    .getBookmarksAsync()
                    // Split the list and emit elements separately.
                    .flatMapIterable((Function<List<Bookmark>, Iterable<Bookmark>>) bookmarks -> bookmarks)
                    // Map bookmarks to the page index they have (that's the only thing we need for filtering).
                    ...
                    .subscribe(new SingleObserver<List<Integer>>() {
                        ...
                        @Override
                        public void onSuccess(List<Integer> integers) {
                            if (integers.isEmpty()) { // If there are no bookmarked pages, we don't have any document to switch to.
                                Toast.makeText(FilterableThumbnailGridActivity.this, "There are no bookmarked pages in the document.", Toast.LENGTH_SHORT).show();
                                return;
                            }
                            ...
                            // Filter the original document to contain just the bookmarked pages, and load it.
    ...
    private void loadOriginalDocument() {
        if (viewModel != null && viewModel.pdfDocument != null) {
            ...
            final DocumentDescriptor originalDocumentDescriptor = DocumentDescriptor.fromDocument(viewModel.pdfDocument);
            documentCoordinator.addOnDocumentVisibleListener(documentDescriptor1 -> getPSPDFKitViews().getThumbnailGridView().show());
     ...
     */
    private void loadFilteredDocument(@NonNull PdfDocument originalDocument, @NonNull Set<Integer> pages, @NonNull String fileSuffix) {
        final PdfProcessorTask pdfProcessorTask = PdfProcessorTask.fromDocument(originalDocument).keepPages(pages);
        ...
        PdfProcessor.processDocumentAsync(pdfProcessorTask, newDocumentFile)
            ...
            .subscribe(new DefaultSubscriber<PdfProcessor.ProcessorProgress>() {
                ...
                public void onNext(PdfProcessor.ProcessorProgress processorProgress) {
                    // Consume processing progress here. Could be used to show the filtering progress in some UI.
                }

                @Override
                public void onError(Throwable t) {
                    // Handle processing error here. Not handled in this particular example.
                }

                @Override
                public void onComplete() {
                    try {
                        DocumentSource documentSource = new DocumentSource(Uri.fromFile(newDocumentFile.getCanonicalFile()));
