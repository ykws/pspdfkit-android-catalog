...
/**
 * This activity showcases how to use {@link DocumentSharingProvider} for exposing shared images via system clipboard.
 * <p>
 * This activity handles image share intents. We use the following intent filter in AndroidManifest.xml:
 * <pre>{@code
 * <intent-filter>
...
public class CopyToClipboardActivity extends AppCompatActivity {
    ...
    private void copySharedImageToSystemClipboard(@NonNull Intent intent) {
        ...
        // Use DocumentSharingProviderProcessor helper to prepare file for sharing in DocumentSharingProvider.
        DocumentSharingProviderProcessor.prepareFileForSharing(this, new ContentResolverDataProvider(imageUri), null)
            ...
            .subscribe((uri, throwable) -> {
                ...
                if (clipboardManager != null && uri != null) {
                    ...
                    ClipData primaryClip = new ClipData(
                        null,
                        new String[]{mimeType},
    ...
    /**
     * Controls whether this activity component is enabled. This setting will override enabled state specified in manifest. Defaults to enabled.
     * @param context   Context to use.
     * @param isEnabled If {@code true}, handling for implicit intents for copying image to clipboard is enabled.
     */
    ...
    public static boolean isEnabled(@NonNull Context context) {
        PackageManager packageManager = context.getPackageManager();
        int componentEnabledSetting = packageManager.getComponentEnabledSetting(new ComponentName(context, CopyToClipboardActivity.class));
