...
public class CustomFormHighlightColorActivity extends PdfActivity {
    ...
    public boolean onCreateOptionsMenu(@NonNull Menu menu) {
        ...
        final Drawable tintedDrawable = DrawableCompat.wrap(ContextCompat.getDrawable(this, R.drawable.pspdf__ic_highlight));
    ...
    /**
     * Toggles form highlight color between {@link Color#TRANSPARENT} and highlight color set in the theme.
     */
    private void toggleFormHighlightColor() {
        if (getConfiguration().getTheme() == PdfActivityConfiguration.NO_THEME) {
            setConfiguration(new PdfActivityConfiguration.Builder(getConfiguration()).theme(R.style.PSPDFCatalog_Theme_FormSelectionNoHighlight).build());
        } else {
            setConfiguration(new PdfActivityConfiguration.Builder(getConfiguration()).theme(PdfActivityConfiguration.NO_THEME).build());
        }
    }
}
