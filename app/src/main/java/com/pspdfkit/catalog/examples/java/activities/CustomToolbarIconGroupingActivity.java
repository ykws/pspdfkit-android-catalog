...
/**
 * This examples shows how to add items to the toolbar and apply a custom grouping rule.
 * See {@link CustomAnnotationCreationToolbarGroupingRule} for the applied grouping.
 */
        ...
        if (toolbar instanceof AnnotationCreationToolbar) {
            ...
            toolbar.setOnMenuItemClickListener((toolbar1, menuItem) -> {
                if (menuItem.getId() == R.id.pspdf_menu_custom) {
                    Toast.makeText(CustomToolbarIconGroupingActivity.this, "Custom Action clicked", Toast.LENGTH_SHORT).show();

                    // You can even hide menu items. This shows how to toggle image menu item by clicking on the custom button.
                    ContextualToolbarMenuItem imageItem = toolbar1.findItemById(R.id.pspdf__annotation_creation_toolbar_item_image);
                    if (imageItem != null) {
                        toolbar1.setMenuItemVisibility(R.id.pspdf__annotation_creation_toolbar_item_image, imageItem.getVisibility() == View.GONE ? View.VISIBLE : View.GONE);
                    }
                    return true;
                }
                return false;
