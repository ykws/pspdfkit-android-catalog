...
public class AnnotationFlagsActivity extends PdfActivity implements ToolbarCoordinatorLayout.OnContextualToolbarLifecycleListener {
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenu.ContextMenuInfo menuInfo) {
        ...
        menu.findItem(R.id.toggle_read_only_flag)
            .setTitle(!annotation.hasFlag(AnnotationFlags.READONLY) ? "Enable Read-Only Flag" : "Disable Read-Only Flag");
        ...
        menu.findItem(R.id.toggle_locked_contents_flag)
            .setTitle(!annotation.hasFlag(AnnotationFlags.LOCKEDCONTENTS) ? "Enable Locked-Contents Flag" : "Disable Locked-Contents Flag");
        ...
        if (annotation.getType() == AnnotationType.NOTE || annotation.getType() == AnnotationType.FILE || annotation.getType() == AnnotationType.STAMP) {
            ...
            noZoomMenuItem.setTitle(!annotation.hasFlag(AnnotationFlags.NOZOOM) ? "Enable No-Zoom Flag" : "Disable No-Zoom Flag");
        } else {
    ...
    public void onPrepareContextualToolbar(@NonNull ContextualToolbar toolbar) {
        // Add item to annotation editing toolbar that will show our context menu for toggling flags on selected annotation.
     ...
     */
    private void setFlagOnAnnotation(@NonNull final Annotation annotation, @NonNull final AnnotationFlags flag, final boolean isSet) {
    ...
    private void setFlagsOnAllAnnotations(@NonNull final EnumSet<AnnotationFlags> flags) {
        getDocument().getAnnotationProvider().getAllAnnotationsOfTypeAsync(EnumSet.allOf(AnnotationType.class))
