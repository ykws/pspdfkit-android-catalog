...
/**
 * Shows how to implement custom set of annotations for
 * {@link com.pspdfkit.internal.ui.dialog.stamps.StampPickerDialog}.
 */
public class CustomStampAnnotationsActivity extends PdfActivity {
    ...
    /**
     * Appearance stream generator instance set via {@link Annotation#setAppearanceStreamGenerator(AppearanceStreamGenerator)} is not
     * retained after reloading the document. Custom appearance stream generated for stamps created via stamp picker item created in
     * {@link #createCustomApStreamStampItem} will be retained in he reloaded document (and in other PDF viewers),
     * however its appearance will be regenerated with default appearance stream after the annotation is modified.
     * This has to be called on the main thread.
     * <p>
     * We register an instance of {@link CustomStampAppearanceStreamGenerator} on {@link AnnotationProvider}
     * to regenerate proper appearance for modified stamps based on their custom subject.
    ...
    /**
     * Shows how to configure annotation defaults and control which properties are going to be displayed in annotation inspector.
     */
    private void configureStampAnnotationsDefaults() {
        ...
        items.add(StampPickerItem.fromPredefinedType(this, PredefinedStampType.ACCEPTED).build());
        items.add(StampPickerItem.fromPredefinedType(this, PredefinedStampType.REJECTED).build());
        ...
        items.add(StampPickerItem.fromTitle(this, "Like").withDateTimeSubtitle(true, true).build());
        ...
        getPdfFragment().getAnnotationConfiguration().put(
            ...
            StampAnnotationConfiguration.builder(this)
                // Here you return list of stamp picker items that are going to be available in the stamp picker.
    ...
    /**
     * This example shows how to create stamp picker item from PDF file containing transparent vector image.
     */
    @NonNull
    private StampPickerItem createCustomApStreamStampItem() {
        ...
        // We want to generate the same AP stream for all stamps with CUSTOM_AP_STREAM_SUBJECT even after the document is reloaded.
        ...
        customStampAppearanceStreamGenerator.addAppearanceStreamGenerator(CUSTOM_AP_STREAM_SUBJECT, appearanceStreamGenerator);
