...
/**
 * This is an example showing how to use the {@link DownloadJob} to download a PDF document from the web with a custom DownloadProgressFragment.
 */
public class CustomDocumentDownloadExample extends PSPDFExample {
    ...
    public static class CustomDownloadProgressFragment extends DownloadProgressFragment {
        ...
        private static String formatProgressSize(Progress progress) {
            ...
            return String.format(Locale.getDefault(), "%.1f/%.1f %sB", progress.bytesReceived / Math.pow((double) unit, (double) exp),
                                 progress.totalBytes / Math.pow((double) unit, (double) exp), pre
            );
        }
        ...
        protected Dialog createDialog() {
            ...
            downloadDescription.setText(getString(R.string.dialog_download_description, getJob().getOutputFile().getName()));
    ...
    public CustomDocumentDownloadExample(@NonNull final Context context) {
        super(context.getString(R.string.customDownloadDialogExampleTitle), context.getString(R.string.customDownloadDialogExampleDescription));
    }
