...
class DocumentJsonExampleActivity : PdfActivity() {
    ...
    private fun exportDocumentJson(uri: Uri) {
        ...
        disposables += DocumentJsonFormatter.exportDocumentJsonAsync(document, outputStream)
    ...
    private fun importDocumentJson(uri: Uri) {
        ...
        disposables += DocumentJsonFormatter.importDocumentJsonAsync(document, ContentResolverDataProvider(uri))
            .subscribeOn(Schedulers.io())
            .observeOn(AndroidSchedulers.mainThread())
            .subscribe({
